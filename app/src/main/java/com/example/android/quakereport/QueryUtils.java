...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray features = rootObject.optJSONArray("features");

            for (int i = 0; i < features.length(); i++) {
                JSONObject earthquake = (JSONObject) features.get(i);
                JSONObject properties = (JSONObject) earthquake.optJSONObject("properties");
                magnitude = properties.optString("mag");
                location = properties.optString("place");
                time = properties.optString("time");
